      ...
      <div className="container mx-auto px-4 py-4">
        <div className="flex items-center justify-between">
          <Link to="/" className="flex items-center space-x-3">
            <img src="/favicon.ico" alt="Pinoy Recipe Finder Logo" className="h-8 w-8" />
          ...
          <nav className="flex items-center space-x-2">
            ...
            {/* Favorites Link */}
            <Link to="/favorites" className={`${location.pathname === "/favorites" ? activeLinkClass : inactiveLinkClass} flex items-center space-x-3`}>
              <span>Favorites</span>
              <span
                className={`rounded-full px-2 py-1 text-xs font-bold w-8 text-center transition-colors duration-300 ${
                  location.pathname === "/favorites" ? "bg-red-600 text-white" : "bg-white text-red-600"
                }`}>
                {favoritesCount}
              </span>
            </Link>
